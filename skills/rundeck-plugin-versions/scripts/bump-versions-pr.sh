...
process_repo() {
  ...
  # Stable branch name (no date) so re-running updates the same PR instead
  # of piling up a new dated branch/PR every day. Check for an existing
  # OPEN PR against it before touching anything - if the last one was
  # merged or closed, this run starts a fresh branch/PR instead of trying
  # to reuse dead history.
  ...
  # Rebuild the branch fresh off current main every run. Bumps always mean
  # "set to latest," so one clean commit each time is simpler and safer
  # than trying to merge/amend across runs - don't hand-edit this branch
  # between runs, it gets discarded and recreated.
  ...
  git -C "$repo_dir" push --force-with-lease origin "$branch" --quiet

  {
    ...
    echo ""
    echo "This branch/PR is regenerated on every run of this script - don't hand-edit the branch, just merge or close the PR."
  } > "$body_file"
  ...
  local pr_title="[RUN-0000] Bump plugin versions to latest releases"
  if [ -n "$existing_pr" ]; then
    gh pr edit "$existing_pr" --repo "$repo_slug" --title "$pr_title" --body-file "$body_file"
    echo "  Updated existing PR: https://github.com/$repo_slug/pull/$existing_pr"
  else
    (cd "$repo_dir" && gh pr create --title "$pr_title" --body-file "$body_file")
  fi
  rm -f "$body_file"
